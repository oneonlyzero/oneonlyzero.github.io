...
```
 1) Use the Strings command on the material will result us any readable strings inside the file, from all of the strings, we clearly there is also a bunch of word that can be a password, maybe of them is the pass that we need to open the flag file.
